...
import fleetjour.scripts.interaction.panel.EntryPanelAssembly.INTEL_OTHER
import fleetjour.scripts.interaction.panel.EntryPanelAssembly.SORTING_TAGS
import java.util.*
...
object EntriesSection {
    ...
    fun renderEntries() {
        ...
        val sectionContainer = parentPanelInstance?.createCustomPanel(PanelConstants.CONTENT_WIDTH,
            sectionHeight, EntryPanelOverseer.plugin)
        val list = this.createEntriesList(sectionContainer, sectionHeight)
    ...
    private fun createEntriesList(section: CustomPanelAPI?, height: Float): TooltipMakerAPI? {
        ...
        val allEntries = this.retrieveFilteredEntries()
